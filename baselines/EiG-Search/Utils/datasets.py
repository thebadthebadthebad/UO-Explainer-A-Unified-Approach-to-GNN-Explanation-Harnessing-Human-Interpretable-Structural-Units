...
class My_Data(Dataset):
    def __init__(self, dataname):
        data_path = f"/home/steve305/kr/mmm/dataset/{dataname}/"
        ...
        self.edge_index = torch.Tensor(edge)
        self.x= torch.Tensor(feature)
        self._num_classes = len(torch.unique(label))
        self.adj = torch.zeros((self.x.shape[0], self.x.shape[0]))
        self.label = torch.FloatTensor(label).long()
        self.y = torch.zeros(self.adj.shape[0], self._num_classes)
        for i in range(self.edge_index.shape[1]):
            n1, n2 = self.edge_index[:, i]
            self.adj[n1, n2] = 1
        for i in range(self.adj.shape[1]):
            self.y[self.label[i]] = 1
        self.y = self.y
