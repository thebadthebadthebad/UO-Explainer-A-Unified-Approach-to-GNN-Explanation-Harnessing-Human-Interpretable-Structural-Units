...
def fidelity(model,node, feature, edge, ground_edge, label):
    label = label[node]
    edge_weight = torch.ones(edge.shape[1], device=args.device)
    ...
    for i in range(edge.shape[1]):
        ...
        if (n1.item(), n2.item()) in ground_edge:
            edge_weight[i] = 0
    raw_predict = torch.softmax(model(feature, edge, edge_weights = edge_weight)[node, :], dim=0)[label]
    predict = torch.softmax(model(feature, edge)[node, :], dim=0)[label]
    return  predict- raw_predict


def find_1_orbit(graph, node, label, model, feature, edge, edge_matrix, choice_n=1000 ):
...
def find_52_orbit(graph, node, label, model, feature, edge, edge_matrix, choice_n = 1000):
    ...
    n = 0
    for a, b, c, d in combination:
        if edge_matrix[a][b] == 0 and edge_matrix[a, c] ==0 and edge_matrix[a,d ] == 0 and edge_matrix[b,c]==1 and edge_matrix[b,d]==1 and edge_matrix[c, d]==1:
            n += 1
...
def find_44_orbit(graph, node, label, model, feature, edge, edge_matrix, choice_n = 1000):
    ...
    for ida, idb, idc in combination:
        ...
        for a, b, c in combi:
            if edge_matrix[b, c] == 0 or edge_matrix[a, b]== 1 or edge_matrix[a,c ] == 1:
                continue
...
def find_73_orbit(graph, node, label, model, feature, edge, edge_matrix, choice_n = 1000):
    neigbor = list(graph.neighbors(node.item()))
    max_fider = -50000
    predict_edge = []
    node = node.item()
...
def find_74_orbit(graph, node, label, model, feature, edge, edge_matrix,choice_n = 1000):
    neigbor = list(graph.neighbors(node.item()))
    max_fider = -50000
    predict_edge = []
    node = node.item()
                ...
                for aa, bb, cc in neighbor_combi:
                    ...
                    for dab, dac in aa_combi:
                        ...
                        for ab, ac in aa_combi_list:
                            ...
                            #print(ground_edge)
                            #df = torch.tensor(ground_edge)
                            #print(torch.unique(df))
                            n += 1
...
def find_75_orbit(graph, node, label, model, feature, edge, edge_matrix, choice_n = 1000):
    neigbor = list(graph.neighbors(node.item()))
    max_fider = -50000
    predict_edge = []
    node = node.item()
                        ...
                        for e, f in d_combi2:
                            ...
                            for dg, dh in e_combi:
                                ...
                                for g, h in e_combi2:
                                    ...
                                    #dss = torch.tensor(ground_edge)
                                    #print(torch.unique(dss))
                                    n += 1
...
def find_76_orbit(graph, node, label, model, feature, edge, edge_matrix, choice_n = 1000):
    neigbor = list(graph.neighbors(node.item()))
    max_fider = -50000
    predict_edge = []
    node = node.item()
