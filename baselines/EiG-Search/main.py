...
from Utils.utils import check_task, load_model, detect_exp_setting, GC_vis_graph, NC_vis_graph, show,my_ground_truth_loader, my_model_selector, make_graph, fidelity
from Utils.metrics import efidelity
from Utils.models import GCN_3l_Gelu, GCN_3l_norm
from torch_geometric.utils import k_hop_subgraph

import sys
sys.path.append('/home/steve305/kr/mmm/other_methods/EiG-Search/Utils/models/')

from Utils.datasets import get_dataset, My_Data
...
def main(args):
    ...
    dataset= My_Data(dataname)

    n_fea, n_cls = dataset.num_features, dataset.num_classes
    explain_ids = detect_exp_setting(dataname, dataset)

    gts, explain_ids = my_ground_truth_loader(dataname)
    gnn_parameter = my_model_selector(dataname).state_dict()
    num_ground_edge = len(gts[0])

    hidden_d = len(gnn_parameter['conv1.bias'])
    if dataname in ['bashapes', 'tree_cycle']:
        gnn_model = GCN_3l_norm(model_level='node', dim_node=dataset.x.shape[1], dim_hidden=hidden_d, num_classes=dataset.num_classes).to(device)
    elif dataname in ['bac', 'tree_grid']:
        #model = GCN_3l_agg(model_level='node', dim_node=node_n, dim_hidden=hidden_d, num_classes=class_n).to(device)
        gnn_model = GCN_3l_Gelu(model_level='node', dim_node=dataset.x.shape[1], dim_hidden=hidden_d, num_classes=dataset.num_classes, num_layer=3).to(device)
    gnn_model.load_state_dict(gnn_parameter)
    ...
    if task_type == "GC":
        ...
        explain_list = []
        print(num_ground_edge)
        for node in explain_ids:
            explain = Hedges[node, :num_ground_edge]
            a = []
            for i in explain:
                a.append((dataset.edge_index[0,i].to('cpu').item(), dataset.edge_index[1,i].to('cpu').item()))
            explain_list.append(a)
        ...
        sparsitys = []
        sub_recalls = []
        for gt, node, explain in zip(gts, explain_ids, explain_list):
            subrecall = 0
            print(gt)
            print(explain)
            recall = len(set(gt)&set(explain))/len(gt)
            if recall == 1:
                subrecall = 1
            fid = fidelity(gnn_model,node, dataset.x, dataset.edge_index, gt, dataset.label)
            fidelitys.append(fid)
            recalls.append(recall)
            sub_recalls.append(subrecall)
            _, edge_index_k_hop, _, _ = k_hop_subgraph(node, 3, dataset.edge_index, relabel_nodes=True)
            n_edges =edge_index_k_hop.size(1)
            sparsity = 1 - len(gt)/n_edges
            sparsitys.append(sparsity)
        print(f'fid: {sum(fidelitys)/len(fidelitys)}')
        print(f'recall: {sum(recalls) / len(recalls)}')
        print(f'sub_recall: {sum(sub_recalls) / len(sub_recalls)}')
        print(f'sparsitys: {sum(sparsitys) / len(sparsitys)}')
    with open(f'result/{dataname}.txt','a' ) as a:
        a.write(f'{dataname} ------------------\n')
        a.write(f'fid: {sum(fidelitys)/len(fidelitys)} \n')
        a.write(f'recall: {sum(recalls) / len(recalls)} \n')
        a.write(f'sub_recall: {sum(sub_recalls) / len(sub_recalls)}\n')
        a.write(f'spar: {sum(sparsitys) / len(sparsitys)}\n\n\n\n\n\n')
        #print(f'sparsitys: {sum(sparsitys) / len(sparsitys)}\n\n\n\n\n\n')
